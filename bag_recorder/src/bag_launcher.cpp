...
BagLauncher::BagLauncher(ros::NodeHandle nh, BLOptions options):
    ...
    record_start_from_python = nh.subscribe("/record/start_python", 1, &BagLauncher::Start_from_Python, this);
    pub_start_from_python = nh.advertise<bag_recorder::Rosbag>("/record/start", 1);
    name_to_python = nh.advertise<std_msgs::String>("/record/bag_name_python", 100);
...
void BagLauncher::Start_Recording(const bag_recorder::Rosbag::ConstPtr& msg){
    ...
    if(publish_name_) {
        ...
        std::string bag_filename_ = data_folder_ + full_bag_name;
        std_msgs::String name;
        name.data = bag_filename_.c_str();
        name_to_python.publish(name);
...
*/
void BagLauncher::Stop_Recording(const std_msgs::Bool::ConstPtr& msg){
    if(msg->data)
    {
    //find recorder in map
    std::map<std::string, std::shared_ptr<BagRecorder>>::iterator recorder = recorders_.find("standard");
    ...
    if(recorder != recorders_.end()) {
        ...
        ROS_INFO("standard configuration recorder stopped.");
    } else {
        ROS_INFO("standard configuration recorder did not exist.");
    }

    if(publish_heartbeat_) {
        //find heartbeat in map
        std::map<std::string, std::shared_ptr<HeartBeat>>::iterator heartbeat = heartbeats_.find("standard");
...
void BagLauncher::Start_from_Python(const std_msgs::String::ConstPtr& msg)
{
    bag_recorder::Rosbag Rosbag_send;
    Rosbag_send.config = "standard";
    Rosbag_send.bag_name = msg->data;
    pub_start_from_python.publish(Rosbag_send);
