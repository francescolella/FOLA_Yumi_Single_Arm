...
{
	ROS_INFO("FromGeometryPoseMsgToRobTarget");
...
{
	ROS_INFO("FromVelocityMsgToSpeedData");
...
Rws::Rws(const ros::NodeHandle &nh) : nh(nh)
{
	...
	_freeDrive_sub = _nh.subscribe("/freeDrive", 0, &Rws::freeDrive_cb, this);
	_cmd_gripper_sub = _nh.subscribe("/gripper", 0, &Rws::gripper_cb, this);
	sub_switch_ = _nh.subscribe("switch_controller", 0, &Rws::switch_contr_cb, this);
	...
	// inizializzazione e calibrazione gripper
	this->p_rws_interface->services().sg().JogOut();
	this->p_rws_interface->services().sg().JogIn();
	this->p_rws_interface->services().sg().Calibrate();
}
...
{
	// ROS_INFO("EGMStartStreamingSignal");
...
{
	// ROS_INFO("EGMStopStreamingSignal");
...
{
	// ROS_INFO("SetControlToFreeDriveSrv");
	if (this->state_machine_state == abb::rws::RWSStateMachineInterface::STATE_FREE_DRIVE_ROUTINE)
	{
...
{
	// ROS_INFO("ReadDigitalInputs"); //debug
...
{
	// ROS_INFO("SetDigitalOutput"); //debug
...
void Rws::LoadControllers(std::vector<std::string> &controllers)
{
	for (std::string controller : controllers)
	{
		...
		controller_manager_m std_srvs::TriggerRequest req;
		std_srvs::TriggerResponse res;
		this->SetControlToEgmSrv(req, res);
		ROS_INFO("Sono nella funzione OFF_freeDrive");
		sgs::LoadController::Response res; // debug
...
{
	// debug
	ROS_INFO("SwitchControllers");
	ROS_INFO("prima dello switch sono nella seguente situazione:");
	this->GetControllersRunning();
...
std::vector<std::string> Rws::GetControllersRunning()
{
	...
	for (auto controller : res.controller)
	{
		...
		{
			ROS_INFO("Controller: %s is in state: %s", controller.name.c_str(), controller.state.c_str()); // debug
...
// Funzioni aggiunte
void Rws::ON_freeDrive()
{
	std_srvs::TriggerRequest req;
	std_srvs::TriggerResponse res;
	this->SetControlToFreeDriveSrv(req, res);
	// ROS_INFO("Sono nella funzione ON_freeDrive"); //debug
}

void Rws::OFF_freeDrive()
{
	std_srvs::TriggerRequest req;
	std_srvs::TriggerResponse res;
	this->SetControlToEgmSrv(req, res);
	// ROS_INFO("Sono nella funzione OFF_freeDrive"); //debug
}

void Rws::freeDrive_cb(std_msgs::Int8ConstPtr data)
{
	// ROS_INFO("la callback mi ha restituito il valore: %d",data->data); //debug
...
void Rws::Open_gripper()
{
	// ROS_INFO("sono in open gripper");
	this->p_rws_interface->services().sg().GripIn();
}

void Rws::Close_gripper()
{
	// ROS_INFO("sono in close gripper");
	this->p_rws_interface->services().sg().GripOut();
}

void Rws::gripper_cb(std_msgs::Int8ConstPtr msg)
{
	// ROS_INFO("E' arrivato il numero: %d",msg->data);
	if (msg->data == 1)
		this->Open_gripper();
	else
		this->Close_gripper();
...
void Rws::switch_contr_cb(std_msgs::BoolConstPtr data)
{
	if (data->data)
	{
		std::vector<std::string> controllers_to_start = {};
		std::vector<std::string> controllers_to_stop = {"VelocityControllers_JointTrajectoryController"};
		this->SwitchControllers(controllers_to_start, controllers_to_stop, 2, false, 0.0);
	}
	else
	{
		std::vector<std::string> controllers_to_start = {"VelocityControllers_JointTrajectoryController"};
		std::vector<std::string> controllers_to_stop = {};
		this->SwitchControllers(controllers_to_start, controllers_to_stop, 2, false, 0.0);
	}
}